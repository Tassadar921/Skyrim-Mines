...
function saveDeposit(deposit: MyDeposit) {
    ...
    router.patch(
        ...
        { resourceId: edit.resourceId, quantity: edit.quantity },
        {
            preserveScroll: true,
            onError: (errors) => {
                toast.error(Object.values(errors)[0] ?? t('deposit.myDeposits.error'));
            },
            onFinish: () => {
                        ...
                        <TableBody>
                            <TableRow v-for="deposit in myDeposits" :key="deposit.id">
                                ...
                                <TableCell>
                                    <Button
                                        size="sm"
                                        :variant="edits[deposit.id].quantity === 0 ? 'destructive' : 'outline'"
                                        :disabled="!isEditable(deposit.createdAt) || savingDepositId === deposit.id"
                                        @click="saveDeposit(deposit)"
                                    >
                                        {{ edits[deposit.id].quantity === 0 ? t('deposit.myDeposits.delete') : t('deposit.myDeposits.save') }}
                                    </Button>
                                </TableCell>
                            </TableRow>
                        </TableBody>
