...
<template>
    <Head :title="t('devis.title')" />
    <div class="min-h-screen pt-32 pb-24 px-[8%] space-y-16">
        <div class="flex flex-col items-center text-center gap-6">
            <h1 class="font-serif text-4xl font-light text-slate-800 dark:text-slate-100">{{ t('devis.title') }}</h1>
            <p class="text-slate-500 dark:text-slate-400 max-w-xl">{{ t('devis.description') }}</p>
        </div>

        <div class="max-w-5xl mx-auto space-y-10">
            <div class="max-w-xl mx-auto space-y-4 rounded-md border p-4">
                <div class="space-y-1">
                    <Label>{{ t('devis.recipient.title') }}</Label>
                    <Select v-model="recipientMode">
                        <SelectTrigger>
                            <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                            <SelectItem v-if="organizationName" value="myOrganization">{{ t('devis.recipient.myOrganization', { name: organizationName }) }}</SelectItem>
                            <SelectItem value="myself">{{ t('devis.recipient.myself') }}</SelectItem>
                            <SelectItem v-if="canRequestForThirdParty" value="thirdParty">{{ t('devis.recipient.thirdParty') }}</SelectItem>
                        </SelectContent>
                    </Select>
                </div>

                <div v-if="recipientMode === 'thirdParty'" class="space-y-1">
                    <Label>{{ t('devis.recipient.thirdPartySelect') }}</Label>
                    <Select v-model="thirdPartySelection">
                        <SelectTrigger>
                            <SelectValue :placeholder="t('devis.recipient.thirdPartyPlaceholder')" />
                        </SelectTrigger>
                        <SelectContent>
                            <SelectGroup v-if="clientsWithOrganization.length">
                                <SelectLabel>{{ t('devis.recipient.clients') }}</SelectLabel>
                            ...
                            <SelectGroup v-if="clientsWithoutOrganization.length">
                                <SelectLabel>{{ t('devis.recipient.clientsPersonal') }}</SelectLabel>
                                <SelectItem v-for="client in clientsWithoutOrganization" :key="client.id" :value="`client:${client.id}`">
                                    {{ client.username }}
                                </SelectItem>
                            </SelectGroup>
                            <SelectGroup>
                                <SelectLabel>{{ t('devis.recipient.organizations') }}</SelectLabel>
                ...
                <div v-if="showScopeChoice" class="space-y-1">
                    <Label>{{ t('devis.recipient.scope') }}</Label>
                    <Select v-model="thirdPartyScope">
                        <SelectTrigger>
                            <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                            <SelectItem value="organization">{{ t('devis.recipient.scopeOrganization', { name: selectedClient?.organizationName ?? '' }) }}</SelectItem>
                            <SelectItem value="personal">{{ t('devis.recipient.scopePersonal') }}</SelectItem>
                        </SelectContent>
                    </Select>
                </div>
            </div>
            ...
            <div class="flex items-center justify-between gap-4 rounded-md border p-4">
                <div class="text-lg">
                    <span class="text-slate-600 dark:text-slate-300">{{ t('devis.totalValue') }}</span>
                    <span class="font-medium text-slate-800 dark:text-slate-100 ml-2">{{ totalAmount.toFixed(2) }} s</span>
                </div>
                <Button :disabled="isSubmitting || !canSubmit" :loading="isSubmitting" @click="submitQuote">
                    {{ t('devis.submit') }}
                </Button>
            </div>
        </div>
