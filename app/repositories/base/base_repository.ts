...
export default class BaseRepository<T extends LucidModel> {
    ...
    public async create(payload: Partial<ModelAttributes<InstanceType<T>>>, trx?: TransactionClientContract): Promise<InstanceType<T>> {
        return trx ? await this.Model.create(payload, { client: trx }) : await this.Model.create(payload);
    }
